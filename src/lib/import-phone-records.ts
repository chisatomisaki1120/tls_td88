  ...
  return db.$transaction(async (tx) => {
    ...
    for (let index = 0; index < dataRows.length; index += 1) {
      ...
      if (seenInFile.has(phoneLast9)) {
        duplicates.push({ rowNumber, phoneRaw, phoneLast9, reason: "Duplicate by last 9 digits in same file" });
        continue;
      }

      const existing = await tx.phoneRecord.findUnique({ where: { phoneLast9 } });
      if (existing) {
        duplicates.push({ rowNumber, phoneRaw, phoneLast9, reason: "Duplicate by last 9 digits", existingRecordId: existing.id });
        continue;
      }

      seenInFile.add(phoneLast9);
      await tx.phoneRecord.create({
        data: {
          phoneRaw,
          phoneLast9,
          assignedStaffId: args.assignedStaffId || null,
