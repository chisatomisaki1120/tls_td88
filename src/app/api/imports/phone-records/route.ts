...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File)) {
      return badRequest("Thiếu file import");
    }
    ...
    if (assignedStaffId) {
      ...
      if (!staff || staff.role !== "staff" || !staff.isActive) {
        return badRequest("Nhân viên nhận data không hợp lệ");
      }
    }

    const buffer = Buffer.from(await file.arrayBuffer());
    const result = await runPhoneRecordImport({
      fileName: file.name,
      fileBuffer: buffer,
      importedByUserId: currentUser.id,
      importedByRole: currentUser.role,
      assignedStaffId,
    });
