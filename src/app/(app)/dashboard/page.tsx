...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Tổng quan"
        description={user?.role === "staff" ? "Chỉ hiển thị danh sách data được gắn cho bạn." : "Snapshot nhanh tình hình dữ liệu số điện thoại."}
      />
      ...
      <Card>
        <div className="mb-4 text-lg font-semibold">
          {user?.role === "staff" ? "Danh sách data của bạn" : "Cập nhật gần đây"}
        </div>
          ...
          {latest.map((item) => (
            <div key={item.id} className="rounded-xl border border-slate-100 px-4 py-3">
              ...
              <div className="mt-1 text-sm text-slate-600">
                {item.statusText || "Chưa có status"} · {item.assignedStaff?.username || "Chưa gán nhân viên"}
              </div>
            </div>
          ))}
